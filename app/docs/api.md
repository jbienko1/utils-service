...
**Cel:** konwersja pliku **DOCX** do Markdown przez [Pandoc](https://pandoc.org/) z opcjonalną normalizacją track changes / komentarzy Word.

**Żądanie:**

- `Content-Type: multipart/form-data`
- pole formularza: **`file`** — tylko `.docx`
- query: **`comments`** — `true` (domyślnie) | `false`
  - `true` — `--track-changes=all`, następnie post-processing: wstawienia bez otoczki, usunięcia jako `~~tekst~~`, komentarze jako blok kodu tuż po skomentowanym fragmencie (`*Autor (data):*` + treść)
  - `false` — `--track-changes=accept` (tekst końcowy bez redakcji i komentarzy)
- query: **`extract_media`** — `false` (domyślnie) | `true`
  - `true` — ekstrakcja obrazów do katalogu `media/`; w Markdown linki `![](media/…)`; w odpowiedzi ZIP (base64) z **jednym** poziomem `media/` w archiwum

**Normalizacja Markdown (GFM):**

- Obrazy z DOCX: `![](media/nazwa.png)` zamiast HTML `<figure>` / `<img>`.
- Podkreślenie z Word → **pogrubienie** (`**tekst**`).
- Proste tabele wielowierszowe → pipe tables.
- Jednowierszowe tabele layoutowe (np. ikona + opis w blockquote) → układ pionowy: obraz, potem cytat (`>`), bez `<table>`.
- Złożone tabele Word (scalone komórki: `colspan`/`rowspan`) mogą pozostać jako HTML (ograniczenie Pandoc).

**Odpowiedź:** `200` — `application/json` — schema `DocxToMarkdownResponse`

| Pole | Typ | Znaczenie |
|------|-----|-----------|
| `markdown` | string | Treść Markdown (GFM) |
| `title` | string lub null | Tytuł z metadanych DOCX (jeśli obecny) |
| `media_count` | integer | Liczba wyekstrahowanych plików graficznych |
| `media_zip_base64` | string lub null | ZIP z katalogiem `media/` (tylko gdy `extract_media=true` i są obrazy) |

**Ograniczenia Pandoc:** komentarze osadzone w niezaakceptowanych wstawieniach mogą nie trafić do wyniku ([pandoc#9833](https://github.com/jgm/pandoc/issues/9833)).
...
|-----|--------|
| `413` | Przekroczony limit rozmiaru uploadu |
| `415` | Plik nie jest `.docx` |
| `503` | Pandoc niedostępny w `PATH` lub błąd konwersji |
